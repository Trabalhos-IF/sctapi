...
Abaixo está o grafo de complexidade ciclomática para os métodos mais relevantes do sistema:
...
\begin{tikzpicture}[node distance=2cm, auto]
    ...
    \node (dec1) [decision, below of=proc1, yshift=-0.5cm] {Estoque Disponível?};
    ...
    \node (proc4) [process, below of=dec2] {Aplicar Promoção};
    \node (dec3) [decision, below of=proc4, yshift=-0.5cm] {Promoção Válida?};
    \node (end) [startstop, below of=dec3] {Fim};

    % Conectando os nós
    \draw [arrow] (start) -- (proc1);
    \draw [arrow] (proc1) -- (dec1);
    \draw [arrow] (dec1) -- node[anchor=south] {Sim} (proc2);
    \draw [arrow] (dec1) -- node[anchor=east] {Não} (proc3);
    \draw [arrow] (proc2) |- (end);
    \draw [arrow] (proc3) -- (dec2);
    \draw [arrow] (dec2) -- node[anchor=east] {Sim} (proc4);
    \draw [arrow] (dec2) -| node[anchor=north] {Não} ([xshift=-2cm]proc3.west) |- (proc1);
    \draw [arrow] (proc4) -- (dec3);
    \draw [arrow] (dec3) -- node[anchor=east] {Sim} (end);
    \draw [arrow] (dec3) -| node[anchor=north] {Não} ([xshift=-2cm]proc4.west) |- (proc3);
\end{tikzpicture}
